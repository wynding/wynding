#!/usr/bin/env node
// check-codex-freshness.mjs — CI guard: "Codex clean" must mean "Codex reviewed THIS head".
//
// The merge gate requires a Codex review of the PR's current head, but Codex does not
// auto-review pushes — it reviews when a PR opens, when a draft goes ready, or when someone
// comments `@codex review`. Between a push and the next trigger there is a window where a
// PR can look "Codex clean" while Codex has only seen an older commit. This check turns
// that window into a red status instead of a silent misread (docs/ai-workflow.md §4).
//
...
// and BOTH carry a `**Reviewed commit:** \`<sha>\`` line. FRESH iff any non-dismissed
// verdict's recorded sha prefix-matches the current PR head. Codex artifacts without a
// parseable sha (e.g. its infrastructure-error comments) never count as verdicts.
//
...
//
// With POST_STATUS=1 (the codex-freshness workflow), the verdict is ALSO posted as a
// `codex-freshness` commit status on the evaluated head — the branch-protection contexts
// list matches statuses as well as job check-runs, and a status can be (re)posted from any
// event, including the `issue_comment` the clean shape arrives as. In that mode a stale
// verdict exits 0: the red status IS the report, and the job stays green so the workflow
// only fails when the check itself could not run.
//
...
const RE_REVIEWED = /Reviewed commit:\s*\*{0,2}\s*`?([0-9a-f]{7,40})/i;
...
if (!/^\d+$/.test(String(prNumber ?? ''))) {
  console.error('❌ codex-freshness: no PR number (set PR_NUMBER or run from a PR event).');
  process.exit(2);
}
...
try {
  ...
  if (fresh) {
    const message = `Codex reviewed head ${short(headSha)} (${fresh.kind} at ${fresh.when})`;
    console.log(`✓ ${message}`);
    if (process.env.POST_STATUS) await postStatus(headSha, 'success', message);
    process.exit(0);
  }
  ...
  console.error(
    latest
      ? `   Latest verdict is a ${latest.kind} for ${short(latest.sha)} at ${latest.when}.`
  ...
  console.error(`❌ codex-freshness: could not determine freshness — ${err.message}`);
