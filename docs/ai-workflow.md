...
Findings are graded [**P0–P3**](CONTEXT.md) by reach. **P0–P2 must be fixed, or the
...
bugs, and ~59% of all findings — but because a loop cannot resolve a thread with a reason,
so gating it on P2 gates it on nearly everything and it never converges. A loop may
